							...
							<div className={shouldShowTab('featureOptions') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										...
										<div className='w-full'>
											<h4 className={`text-base`}>Agent Pipeline</h4>
											<div className='text-sm text-void-fg-3 mt-1'>{`Enable the two-phase Agent Pipeline (planning then execution).`}</div>

											<div className='my-2'>
												{/* Enable Pipeline Switch */}
												<ErrorBoundary>
													<div className='flex items-center gap-x-2 my-2'>
														<VoidSwitch
															size='xs'
															value={settingsState.globalSettings.agentPipelineEnabled}
															onChange={(newVal) => voidSettingsService.setGlobalSetting('agentPipelineEnabled', newVal)}
														/>
														<span className='text-void-fg-3 text-xs pointer-events-none'>{settingsState.globalSettings.agentPipelineEnabled ? 'Enabled' : 'Disabled'}</span>
													</div>
												</ErrorBoundary>
												...
												{settingsState.globalSettings.agentPipelineEnabled && (
													<div className="flex flex-col gap-2 mt-2">
														<div className="flex items-center gap-2">
															<span className="text-void-fg-3 text-xs w-24">Planning Model:</span>
															<VoidSimpleInputBox
																value={settingsState.globalSettings.agentPlanningModelOverride}
																onChangeValue={(v) => voidSettingsService.setGlobalSetting('agentPlanningModelOverride', v)}
																placeholder="e.g. qwen2.5-coder:7b (leave blank for Chat model)"
																compact={true}
															/>
														</div>
														<div className="flex items-center gap-2">
															<span className="text-void-fg-3 text-xs w-24">Execution Model:</span>
															<VoidSimpleInputBox
																value={settingsState.globalSettings.agentExecutionModelOverride}
																onChangeValue={(v) => voidSettingsService.setGlobalSetting('agentExecutionModelOverride', v)}
																placeholder="e.g. qwen2.5-coder:3b (leave blank for Chat model)"
																compact={true}
															/>
														</div>
													</div>
