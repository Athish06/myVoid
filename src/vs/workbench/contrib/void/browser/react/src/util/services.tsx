...
let agentPipelineState: PipelineState | null = null
const agentPipelineStateListeners: Set<(s: PipelineState) => void> = new Set()
...
export const _registerServices = (accessor: ServicesAccessor) => {
	...
	const stateServices = {
		...
		agentPipelineService: accessor.get(IAgentPipelineService),
	}

	const { settingsStateService, chatThreadsStateService, refreshModelService, themeService, editCodeService, voidCommandBarService, modelService, mcpService, agentPipelineService } = stateServices
	...
	agentPipelineState = agentPipelineService.state
	disposables.push(
		agentPipelineService.onDidChangePipelineState(() => {
			agentPipelineState = agentPipelineService.state
			agentPipelineStateListeners.forEach(l => l(agentPipelineState!))
		})
	)
...
export const useAgentPipelineState = () => {
	const [s, ss] = useState(agentPipelineState)
	useEffect(() => {
		ss(agentPipelineState)
		const listener = (newState: PipelineState) => { ss(newState) }
		agentPipelineStateListeners.add(listener)
		return () => { agentPipelineStateListeners.delete(listener) }
	}, [ss])
	return s
}
